...
Веса Amazon Chronos-Bolt Tiny уже включены в репозиторий в безопасном формате `safetensors`. Эта модель работает полностью offline и не требует доступа корпоративной виртуальной машины к Hugging Face. Более крупные pretrained-модели остаются внешними из-за лимита GitHub на размер одного файла.

На корпоративном запуске `start-corp.sh` отключает Xet-транспорт Hugging Face и использует обычную HTTPS-загрузку с увеличенным timeout. Это помогает, когда страница Hugging Face доступна, но корпоративный proxy блокирует отдельный Xet/CAS-трафик больших файлов. Статус и последняя ошибка скачивания показываются непосредственно на карточке модели.

LightGBM не входит в обязательные зависимости, поскольку корпоративные PyPI-репозитории могут блокировать этот пакет. Приложение и остальные модели работают без него. Если пакет разрешён, его можно добавить отдельно:
