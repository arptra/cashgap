...
export default function RunModelPage() {
  ...
  const environmentsReady = selectedDefinitions.every((model) => model?.environment.installed && model.environment.dependency_installed)
  ...
  return <>
    ...
    <section className="wizard-grid">
      ...
      <article className="panel wizard-step wizard-models"><b>4</b><h2>Модели</h2><div className="wizard-model-grid">{candidates.map((model) => { const ready = model.environment.installed && model.environment.dependency_installed; return <label className={`${selectedModels.includes(model.id) ? 'selected' : ''} ${!ready ? 'disabled' : ''}`} key={model.id}><input type="checkbox" checked={selectedModels.includes(model.id)} disabled={!ready} onChange={() => toggle(model.id)} /><span>{model.name}<small>{model.requires_training ? 'обучение' : 'zero-shot'} · {model.environment.status}</small></span></label> })}</div></article>
